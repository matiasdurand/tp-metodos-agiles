...
public class ClassGValidator implements Validator<LicenseType,Titular> {
	...
	public List<LicenseType> validate(Titular info) {
		...
		List<License> licensesTypeG = info.getLicenses().stream()
				.filter(l -> l.getLicenseType().equals(LicenseType.G)).collect(Collectors.toList());
				
		if(licensesTypeG.isEmpty()) {
			...
			if(ChronoUnit.YEARS.between(birthday, actual)>17) {
				validTypesToShow.add(LicenseType.G);
			}
		}
